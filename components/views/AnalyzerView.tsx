...
export default function AnalyzerView({ topics }: AnalyzerViewProps) {
  ...
  const renderInteractiveText = () => {
        ...
        {words.map((word, index) => {
          ...
          if (appState === 'analyzing') {
            wordClasses += isSelected 
              ? "bg-[#7c1f31]/40 text-[#1a1a1a] cursor-pointer" 
              : "cursor-pointer hover:bg-[#1a1a1a]/10";
          } else if (appState === 'results') {
            if (isSelected && isToxic) {
              wordClasses += "bg-green-200 text-green-900 font-medium font-sans";
            } else if (!isSelected && isToxic) {
              wordClasses += "bg-red-100 text-red-900 underline decoration-red-500 decoration-2 underline-offset-4 font-sans";
            } else if (isSelected && !isToxic) {
              wordClasses += "bg-[#1a1a1a]/20 text-[#1a1a1a]/60 line-through decoration-[#1a1a1a]/40 font-sans";
            } else {
    ...
    if (uiStyle === 'facebook') {
      ...
      return (
        <div className="max-w-2xl mx-auto bg-[#fdfdfc] rounded-sm shadow-sm border border-gray-300 p-8 md:p-12 font-serif">
          <div className="border-b-2 border-black pb-4 mb-6">
            <h1 className="text-3xl font-bold text-black mb-2 uppercase tracking-tight">The Daily Chronicle</h1>
            <div className="flex items-center justify-between text-xs text-gray-600 font-sans uppercase tracking-widest">
              <span>Published Today</span>
              <span>By Editorial Board</span>
            </div>
          </div>
          <div className="text-xl md:text-2xl leading-loose md:leading-loose text-[#1a1a1a]">
            {textContent}
          </div>
        </div>
      );
          ...
          {appState === 'select-topic' && (
            ...
            >
              <div className="text-center space-y-4">
                <h2 className="text-4xl font-extrabold tracking-tight text-[#1a1a1a]">Laboratorul de Adevăr</h2>
                ...
                {topics.map((topic) => {
                  ...
                  return (
                    <button
                      ...
                      className="group relative flex flex-col items-start p-8 bg-white rounded-3xl border border-[#1a1a1a]/10 shadow-sm hover:shadow-md hover:border-[#7c1f31] transition-all text-left min-h-[44px]"
                    >
                      ...
                      <h3 className="font-bold text-xl mb-3 text-[#1a1a1a]">{topic.title}</h3>
                      <p className="text-base text-[#1a1a1a]/70 leading-relaxed">{topic.description}</p>
                    </button>
                  );
                })}
              </div>
          ...
          {(appState === 'analyzing' || appState === 'results') && articleData && (
            ...
            >
              <div className="space-y-3 text-center md:text-left">
                <div className="inline-flex items-center rounded-full border border-transparent bg-[#7c1f31]/10 px-4 py-1.5 text-sm font-bold text-[#7c1f31]">
              ...
              {appState === 'analyzing' && (
                <div className="bg-white p-6 rounded-2xl border border-[#1a1a1a]/10 shadow-sm space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 font-bold text-lg text-[#1a1a1a]">
                      <Clock className="w-5 h-5 text-[#7c1f31]" />
                      Timp rămas: <span className={timeLeft <= 5 ? "text-red-600" : ""}>{timeLeft}s</span>
                    </div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-1000 ease-linear ${timeLeft <= 5 ? 'bg-red-500 animate-pulse' : 'bg-[#7c1f31]'}`}
              ...
              {appState === 'results' && (
                <motion.div 
                  ...
                  className="space-y-8"
                >
                  <div className="bg-white p-8 md:p-10 rounded-3xl border border-[#1a1a1a]/10 shadow-lg">
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                      <div className="flex-1 space-y-6">
                        <div className="flex items-center gap-4">
                          <div className="w-20 h-20 rounded-full bg-[#7c1f31]/10 flex items-center justify-center text-3xl font-black text-[#7c1f31]">
                            {accuracy}%
                          </div>
                          <div>
                            <h3 className="text-2xl font-bold text-[#1a1a1a]">Acuratețe</h3>
                            <p className="text-[#1a1a1a]/70">Ai identificat corect {accuracy}% din cuvintele toxice.</p>
                          </div>
                        </div>
                        
                        <div>
                          <h3 className="text-2xl font-bold text-[#1a1a1a] mb-3 flex items-center gap-2">
                            <Brain className="w-6 h-6 text-[#7c1f31]" />
                            Explicația AI-ului
                          </h3>
                          <p className="text-lg text-[#1a1a1a]/80 leading-relaxed bg-[#e7edeb] p-6 rounded-2xl border border-[#7c1f31]/20">
                      ...
                      <div className="w-full md:w-1/3 space-y-6">
                        <div className="bg-[#e7edeb] p-6 rounded-2xl border border-[#1a1a1a]/10">
                          <h4 className="font-bold text-[#1a1a1a] mb-4">Radar Emoțional</h4>
                          <div className="space-y-3">
                            {Object.entries(articleData.emotions || {}).map(([emotion, score]) => (
                              <div key={emotion}>
                                <div className="flex justify-between text-sm font-medium text-[#1a1a1a] mb-1 capitalize">
                                  <span>{emotion === 'urgency' ? 'Urgență' : emotion === 'validation' ? 'Validare' : emotion === 'fear' ? 'Frică' : 'Furie'}</span>
                                  <span>{score}%</span>
                                </div>
                                <div className="w-full bg-white rounded-full h-2">
                                  <div className="bg-[#7c1f31] h-2 rounded-full" style={{ width: `${score}%` }}></div>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="bg-[#e7edeb] p-6 rounded-2xl border border-[#1a1a1a]/10 flex flex-col h-full">
                          <h4 className="font-bold text-[#1a1a1a] mb-4 flex items-center gap-2">
                            <Brain className="w-5 h-5 text-[#7c1f31]" />
                  ...
                  <div className="flex justify-center pt-6">
                    <button 
                      ...
                      className="inline-flex items-center justify-center rounded-xl text-lg font-bold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1a1a1a] bg-white border-2 border-[#1a1a1a] text-[#1a1a1a] hover:bg-[#1a1a1a] hover:text-white h-14 px-10 gap-3 min-h-[44px]"
                    >
